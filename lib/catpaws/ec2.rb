...
Capistrano::Configuration.instance(:must_exist).load do
    ...
    set (:ec2) { AWS::EC2::Base.new( :access_key_id => access_key, :secret_access_key => secret_access_key, :server => server) }
    ...
    task :start, :hosts => master do
      ...
      ridfile       = variables[:ridfile] or  abort "Specify an rid (reservation id) file to keep track of the instances you're using. For example cap start_ec2 -S ridfile='filename'"
      nhosts        = variables[:nhosts] || 1
      ami           = variables[:ami] or abort "Please specify an ami to use: cap start_ec2 -S ami='ami123456' "
      instance_type = variables[:instance_type] or abort "Please specify an instance_type to use: cap start_ec2 -S instance_type='m1.small'"
      key           = variables[:key] or abort "Please specify an AWS key to use: cap start_ec2 -S key='mykey' "
